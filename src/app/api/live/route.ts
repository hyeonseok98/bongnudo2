...
export async function GET() {
  try {
    const channelIds = await getParticipantChannelIds();
    const broadcasts = await getChzzkLiveBroadcasts(channelIds);
...
async function getParticipantChannelIds(): Promise<Set<string>> {
  ...
  return new Set(
    result.data.flatMap(getParticipantChannelId),
  );
}

function getParticipantChannelId(
  participant: ParticipantChannelQueryData[number],
): string[] {
  return participant.streamer.chzzk_channel_id
    ? [participant.streamer.chzzk_channel_id]
    : [];
}

async function getChzzkLiveBroadcasts(
  participantChannelIds: Set<string>,
  ...
  do {
    ...
    for (const live of parsedResponse.content.data) {
      if (participantChannelIds.has(live.channelId)) {
        broadcasts.push({
          liveId: live.liveId,
