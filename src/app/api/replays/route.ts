...
import { parseReplayCursor } from "@/features/replays/replay-cursor";
import { getReplayPage } from "@/features/replays/get-replays";
...
export async function GET(request: Request) {
  ...
  if (!filters) {
    return NextResponse.json({ message: "다시보기 필터가 올바르지 않음." }, { status: 400 });
  }

  const cursorValue = searchParams.get("cursor");
  const cursor = parseReplayCursor(cursorValue);

  if (cursorValue && cursor === null) {
    return NextResponse.json({ message: "다시보기 페이지 정보가 올바르지 않음." }, { status: 400 });
  }

  try {
    const page = await getReplayPage(filters, cursor);

    return NextResponse.json(page, { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
    console.error("Failed to load replays", error);

    return NextResponse.json({ message: "다시보기를 불러오지 못함." }, { status: 500 });
  }
}

function parseFilters(searchParams: URLSearchParams): ReplayListFilters | null {
  ...
  if (
    ...
    participantIds === null ||
    (dayValue !== null &&
      (!/^\d+$/.test(dayValue) || !Number.isSafeInteger(Number(dayValue)) || Number(dayValue) < 1)) ||
