...
const EXPECTED_CAREER_EVENT_HEADERS = [
  "인물",
  "event_type",
  "from_organization",
  "to_organization",
...
export async function readBongnudo2Excel(workbookPath) {
  ...
  const careerEventRows = readOptionalRows(workbook, "RP이력");
  ...
  if (careerEventRows.isPresent) {
    validateHeaders(careerEventRows.headers, EXPECTED_CAREER_EVENT_HEADERS, "RP이력");
  }
  ...
  const careerEvents = careerEventRows.rows.map(normalizeCareerEvent);
  ...
  const errors = validateWorkbook({
    ...
    applications,
    careerEvents,
    hasCareerEventsSheet: careerEventRows.isPresent,
  });
...
function readOptionalRows(workbook, sheetName) {
  const sheet = workbook.getWorksheet(sheetName);

  if (!sheet) {
    return { headers: [], rows: [], isPresent: false };
  }

  return { ...readRows(workbook, sheetName), isPresent: true };
}
...
function normalizePerson(row, organizationNames) {
  ...
  return {
    ...
    birthDate: toDateText(row.birth_date),
    hasStatedAge,
    affiliations,
    roleHistories,
    statedAge:
      !hasStatedAge
        ? null
        : toPositiveInteger(row.stated_age),
  };
}

function normalizeCareerEvent(row) {
  const hasEventTime = row.event_time !== null && toText(row.event_time) !== "";
  const hasSeasonDayNumber =
    row.season_day_number !== null && toText(row.season_day_number) !== "";
  const eventDate = toDateText(row.event_date);
  const eventTime = hasEventTime ? toTimeText(row.event_time) : null;

  return {
    row: row.__row,
    personName: toText(row["인물"]),
    eventType: toText(row.event_type).toLowerCase(),
    fromOrganization: toText(row.from_organization) || null,
    toOrganization: toText(row.to_organization) || null,
    fromRole: toText(row.from_role) || null,
    toRole: toText(row.to_role) || null,
    eventDate,
    eventTime,
    hasEventTime,
    eventAt: eventDate && eventTime ? `${eventDate}T${eventTime}:00+09:00` : null,
    sequenceInDay: toPositiveInteger(row.sequence_in_day),
    seasonDayNumber:
      !hasSeasonDayNumber ? null : toPositiveInteger(row.season_day_number),
    hasSeasonDayNumber,
    note: toText(row.note) || null,
    sourceUrl: toText(row.source_url) || null,
  };
}
...
function validateWorkbook(data) {
  ...
  validateUnique(
    data.careerEvents,
    (event) =>
      event.personName && event.eventDate && event.sequenceInDay
        ? `${event.personName}\u0000${event.eventDate}\u0000${event.sequenceInDay}`
        : "",
    "RP이력",
    "같은 인물·날짜·순번의 사건 중복",
  ...
  validateCareerEventSequence(data.careerEvents, errors);
  validateUnique(
    data.people,
  ...
  for (const person of data.people) {
    ...
    if (!isValidDate(person.birthDate)) {
      addError(errors, "인물", person.row, "birth_date 형식 오류");
    }
    if (person.statedAge === null && person.hasStatedAge) {
      addError(errors, "인물", person.row, "stated_age는 양의 정수여야 함");
    }
  ...
  for (const event of data.careerEvents) {
    if (!personNames.has(event.personName)) {
      addError(errors, "RP이력", event.row, "인물 시트에 없는 인물", event.personName);
    }
    if (!CAREER_EVENT_TYPES.has(event.eventType)) {
      addError(errors, "RP이력", event.row, "event_type 오류", event.eventType);
    }
    if (!event.eventDate || !isValidDate(event.eventDate)) {
      addError(errors, "RP이력", event.row, "event_date 형식 오류");
    }
    if (!event.sequenceInDay) {
      addError(errors, "RP이력", event.row, "sequence_in_day는 양의 정수여야 함");
    }
    if (event.fromOrganization && !organizationNames.has(event.fromOrganization)) {
      addError(errors, "RP이력", event.row, "from_organization이 조직목록에 없음", event.fromOrganization);
    }
    if (event.toOrganization && !organizationNames.has(event.toOrganization)) {
      addError(errors, "RP이력", event.row, "to_organization이 조직목록에 없음", event.toOrganization);
    }
    if (event.eventTime === null && event.hasEventTime) {
      addError(errors, "RP이력", event.row, "event_time 형식 오류");
    }
    if (event.seasonDayNumber === null && event.hasSeasonDayNumber) {
      addError(errors, "RP이력", event.row, "season_day_number는 양의 정수여야 함");
    }
    if (!hasValidCareerEventOrganizations(event)) {
      addError(errors, "RP이력", event.row, "event_type에 필요한 조직 정보 누락");
    }
    if (event.sourceUrl && !isHttpUrl(event.sourceUrl)) {
      addError(errors, "RP이력", event.row, "source_url 형식 오류", event.sourceUrl);
    }
  }
...
function buildSummary(data) {
  ...
  return {
    ...
    careerEvents: data.careerEvents.length,
...
function hasValidCareerEventOrganizations(event) {
  switch (event.eventType) {
    case "join":
      return event.toOrganization !== null;
    case "leave":
      return event.fromOrganization !== null;
    case "transfer":
      return event.fromOrganization !== null && event.toOrganization !== null;
    case "demote":
    case "promote":
      return event.fromOrganization !== null || event.toOrganization !== null;
    case "appoint":
      return event.toOrganization !== null || event.fromOrganization !== null;
    default:
      return false;
  }
}

function validateCareerEventSequence(events, errors) {
  const eventsByPersonAndDate = new Map();

  for (const event of events) {
    if (!event.personName || !event.eventDate || !event.sequenceInDay) {
      continue;
    }

    const key = `${event.personName}\u0000${event.eventDate}`;
    eventsByPersonAndDate.set(key, [
      ...(eventsByPersonAndDate.get(key) ?? []),
      event,
    ]);
  }

  for (const groupedEvents of eventsByPersonAndDate.values()) {
    const orderedEvents = [...groupedEvents].sort(
      (left, right) => left.sequenceInDay - right.sequenceInDay,
    );
    ...
    for (const event of orderedEvents) {
      ...
      if (previousEventAt !== null && event.eventAt < previousEventAt) {
        addError(
          errors,
          "RP이력",
          event.row,
          "sequence_in_day와 event_time 순서가 맞지 않음",
        );
      }
      previousEventAt = event.eventAt;
    }
  }
}
