...
export async function loadSyncContext(client, excelData, requestedSeasonId) {
  ...
  const [streamersResult, participantsResult, organizationsResult, affiliationsResult, affiliationMembershipsResult, organizationMembershipsResult, historiesResult, jobsResult, recruitmentsResult, sessionsResult, applicationsResult] = await Promise.all([
  ...
  const matchErrors = people
    .filter(({ streamer, participant }) => !streamer || !participant)
    .map(({ person, streamer }) =>
      streamer
        ? `${person.name}: 대상 시즌 참가자를 찾지 못함.`
        : `${person.name}: 치지직 채널 ID로 스트리머를 찾지 못함.`,
    );
  if (matchErrors.length > 0) {
    throw new Error(
      `DB 매칭 실패: ${matchErrors.length}건\n${matchErrors.slice(0, 30).join("\n")}`,
    );
  }
...
export function buildSyncPlan(excelData, context) {
  const desired = buildDesiredState(excelData, context);
  const sourceStreamerIds = new Set(context.people.map(({ streamer }) => streamer.id));
  const sourceParticipantIds = new Set(context.people.map(({ participant }) => participant.id));
  ...
  const currentPeople = context.people.map(({ person, streamer, participant }) => ({
    key: participant.id,
    label: person.name,
...
export async function applySyncPlan(client, plan, force) {
  ...
  const afterPlan = buildSyncPlan({ ...plan.excelData, people: plan.excelData.people }, refreshed);
...
async function updatePeople(client, rows) {
  for (const row of rows) {
    const { error: participantError } = await client.from("season_participants").update({ rp_name: row.rpName }).eq("id", row.key);
