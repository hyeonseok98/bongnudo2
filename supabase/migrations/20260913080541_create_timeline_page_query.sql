create function public.get_timeline_page(p_filters jsonb)
returns jsonb
language sql
stable
security invoker
...
as $$
        ...
        or exists (
          ...
          where event_participant.timeline_event_id = event.id
            and membership.affiliation_id in (
              select selected_affiliations.id
  ...
  event_rows as (
    ...
    left join lateral (
      select jsonb_agg(
        jsonb_build_object(
          ...
          'profileImageKey', coalesce(
            participant.portrait_image_key,
            streamer.profile_image_key
          ),
    ...
    left join lateral (
      select jsonb_agg(
        ...
        order by event_media.sort_order
      ) as items
  ...
  popular_tags as (
    select
      tag.slug,
      tag.name,
      count(distinct event_tag.timeline_event_id) as usage_count
    from scoped_events as event
    join public.timeline_event_tags as event_tag
      on event_tag.timeline_event_id = event.id
    join public.timeline_tags as tag
      on tag.id = event_tag.tag_id
