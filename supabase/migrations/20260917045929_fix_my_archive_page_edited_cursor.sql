...
begin
  ...
  if p_tab = 'edited' then
    ...
    page as (
      select edited.*
      from edited_archives as edited
      where (
        p_cursor_at is null
        or (edited.last_edited_by_me_at, edited.id) < (p_cursor_at, p_cursor_id)
      )
      order by edited.last_edited_by_me_at desc, edited.id desc
      limit v_limit
    )
    select
      page.id,
